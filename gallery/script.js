const fs = require('fs');
let template = fs.readFileSync('template.html', 'utf8').split('`');

let galleries = fs.readdirSync('.');

for(index = galleries.length - 1;index >=0;index--){ //have to loop goofy so I can kill elements
    if(!RegExp('^\.[^.]+$').test(galleries[index])) //filter out items that arent directories
...
galleries.forEach(gallery=>{
    ...
    fs.copyFile ('style.css','./'+gallery+'/style.css',(err) => {  
        if (err) throw err;
